 ...
 */
public final class ServicoProgressao {

    public static final String VERSAO_BALANCEAMENTO = "BAL-1.0.0";

    private static final double XP_A = 50.0;
    private static final double XP_B = 10.0;
    private static final double XP_C = 10.0;

    private ServicoProgressao() {}
     ...
     */
    public static long xpTotalParaNivel(int n) {
        if (n < 1) throw new IllegalArgumentException("Nível deve ser >= 1, recebido: " + n);
        long total = 0;
        for (int i = 1; i <= n; i++) {
            total += xpParaNivel(i);
     ...
     */
    public static boolean deveSubirNivel(long xpAccumulated, int currentLevel) {
        if (currentLevel < 1) throw new IllegalArgumentException("Nível atual deve ser >= 1");
        return xpAccumulated >= xpTotalParaNivel(currentLevel + 1);
    }
     ...
     */
    public static int totalPeaNoNivel(int level) {
        if (level < 2) return 0;
     ...
     */
    public static int peaAoSubirDeNivel(int newLevel) {
        if (newLevel < 2) return 0;
        return totalPeaNoNivel(newLevel) - totalPeaNoNivel(newLevel - 1);
    }
     ...
     */
    public static int totalPpNoNivel(int level) {
     ...
     */
    public static int ppAoSubirDeNivel(int newLevel) {
     ...
     */
    public static int bonusProficiencia(int level) {
     ...
     */
    public static int valorFinalHabilidade(int atributoBase, int ppDaHabilidade, int level, int modificadores) {
        return atributoBase + ppDaHabilidade + bonusProficiencia(level) + modificadores;
    }

    /**
     * Limite suave de atributo por nível: 10 + floor(level / 5) — RF-PP-05.
     */
    public static int limiteSupaveAtributo(int level) {
        return 10 + level / 5;
    }
     ...
     */
    public static boolean slotHabilidadeDesbloqueado(int level, int slotIndex) {
     ...
     * Marco: nível 12.
     */
    public static boolean habilidadeAssinaturaDesbloqueada(int level) {
        return level >= 12;
    }
     ...
     * Marco: nível 20.
     */
    public static boolean especializacaoAvancadaDesbloqueada(int level) {
        return level >= 20;
    }
     ...
     * Marco: nível 30+.
     */
    public static boolean cicloMaestriaAtivo(int level) {
        return level >= 30;
    }
}
